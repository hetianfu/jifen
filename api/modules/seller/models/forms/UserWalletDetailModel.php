...
class UserWalletDetailModel extends Base{
    ...
    public function rules(){
        ...
        return array_merge([
            ...
            [['id','user_id','type','real_name','pay_type','bank_code','bank_address','alipay_code','remark', 'open_id','path','operator'],'string','on' => [ 'default','update']],
    ...
    public function add(){
        ...
        if( $result ){
            ...
            $event->content=  WalletStatusEnum::getDescribe($event->type). $event->amount .'元。'.$this->remark;
            $event->insert();
        }
        return $result;
    }
