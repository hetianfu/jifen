 ...
 * @property string $real_name; 用户名字
...
class UserCommissionDetailModel extends Base{
    ...
    public function rules(){
        ...
        return array_merge([
            ...
            [[ 'user_id','open_id','type','provider_id','order_id','pay_type','fail_msg','real_name', 'remark',],'string','on' => [ 'default','update']],
            [['is_deduct','status','level' ], 'integer','on' => [ 'default','update']],
            [['amount', ], 'number','on' => [ 'default','update']],
            [['detail', ], 'safe','on' => [ 'default','update']],
    ...
    public function fields(){
        ...
        isset($this->detail)&& $fields['detail']= function (){
    ...
    public function getUserSnap(){
        return $this->hasOne( UserInfoModel::class,['id'=>'provider_id'])->select(['id','nick_name','code','head_img','telephone' ]);
    }

    public function getOrderInfo(){
        return $this->hasOne( BasicOrderInfoModel::class,['id'=>'order_id'])->select(['id','cart_snap','pay_amount'  ]);
    }
