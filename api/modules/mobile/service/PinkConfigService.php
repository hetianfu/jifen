...
/*********************PinkConfig模块服务层************************************/
    ...
    public function getPinkProductIds(PinkConfigQuery $query)
    {
        ...
        $searchModel = new SearchModel([
            ...
            'scenario' => 'default',
            'partialMatchAttributes' => ['name'], // 模糊查询
            'select' => ['product_id','start_time'], //查商品Id
            'distinct'=>true,
            'defaultOrder' => $query->getOrdersArray(),
            'pageSize' => $query->limit,
        ]);
        ...
        return implode(',',array_column(  ArrayHelper::toArray($searchWord->getModels()),'product_id') ) ;
    }
    public function getPinkPidAndStartList(PinkConfigQuery $query)
    {
        $query->end_time=QueryEnum::GT.time();
        $searchModel = new SearchModel([
            'model' => PinkConfigModel::class,
