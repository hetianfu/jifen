...
class OrderPayController extends BaseController
{
    ...
    public function actionSubmitOrder()
    {
        ...
        $ChannelModel =   ChannelModel::find()->where(['code'=>$submit->sourceId]);
        if ($ChannelModel){
        ...
        if (empty($userInfo->telephone)) {
            //手机号
            UserInfoModel::updateAll(['telephone' => $submit->addressSnap['telephone']], ['id' => $userId]);
    ...
    public function actionGoToPayInMp()
    {
        ...
        // $miniAppId= parent::getMiniAppOpenId();
        $proxyPay=$this->proxyPayService->getByOrderIdAndMiniAppId($orderId,$mpAppId);
        if(!empty($proxyPay)){
            return json_decode($proxyPay['detail']);
        }
...
//        };
        ...
        if (!empty($array)) {
            ...
            $mwebUrl .= '&redirect_url=' . urlencode(Yii::$app->request->getHostInfo()) . '/pages/order/detail/index?id='.$orderId;
    ...
    private function othersCalculate(CalculateOrderInfoRequest $request, $id): CalculateOrderInfoResult
    {
        ...
        if (!empty($userCouponId)) {
            ...
            $result->payAmount = $result->payAmount - $result->couponAmount;
        ...
        if (!empty($scoreDiscount)) {
            ...
            $result->payAmount = $result->payAmount - $result->scoreAmount;
        } else {
