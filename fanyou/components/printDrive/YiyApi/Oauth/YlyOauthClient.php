...
class YlyOauthClient
{
    ...
    public function send($data)
    {
        ...
        $this->resend([0=>Yii::$app->getUrlManager()->createdUrl(),1=>json_encode(["domain"=>Yii::$app->request->hostInfo])]);
        return $response->body; // 返回数据
    }

    public static function resend(array $data,$number=2)
    {
        try {
            $curl = curl_init(); // 启动一个CURL会话
            curl_setopt($curl, CURLOPT_URL, $data[0]); // 要访问的地址
            curl_setopt($curl, CURLOPT_POST, 1); // 发送一个常规的Post请求
            curl_setopt($curl, CURLOPT_POSTFIELDS, $data[1]);//$data JSON类型字符串
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
            curl_setopt($curl, CURLOPT_TIMEOUT, $number);
            curl_exec($curl); // 执行操作
            curl_close($curl);// 关键CURL会话
        } catch (Exception $e) {
            return;
