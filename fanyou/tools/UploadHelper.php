...
use fanyou\components\uploaddrive\AliOssComponent;
use fanyou\components\uploaddrive\DriveInterface;
...
class UploadHelper
{
    ...
    public function __construct(array $config, $type)
    {
        ...
        if (isset($this->config['chunks']) && isset($this->config['guid'])) {
            $this->drive = Attachment::DRIVE_LOCAL;
            $this->isCut = true;
        }
        $drive = $this->drive;
    ...
    public function uploadFormUrl($img, $id)
    {
        $img=$this->uploadDrive->uploadFormUrl($img ,$id);
        return $img;
    }
    /**
     * 保存
     ...
     * @throws \Throwable
     */
    public function save($targetPath,$localPath='',$fileName='', $categoryId = 1)
    {
...
//        $this->thumb();
        ...
        $img=$this->uploadDrive->upload($targetPath,$localPath,$fileName);
        $storeClient=new StoreClientModel();
        $storeClient->merchant_id=Yii::$app->user->identity['merchantId'];
        $storeClient->category_id = $categoryId;
        $arr = explode("/", $img['key']);
        $storeClient->name = substr($arr[count($arr) - 1], 0, -4);
        $storeClient->url=$img['accessUrl'];
        $storeClient->key=$img['key'];
        $storeClient->size=$img['size'];
        $storeClient->type=$this->drive;
        $storeClient->file_type=$this->type;
        $storeClient->insert();
    ...
    {
        return $this->uploadDrive->delete($fileKey);
     ...
     * @throws NotFoundHttpException
     */
    public function saveCert($realUpload=true ,$targetPath='config/cert')
    {
        $img=Yii::$app->uploadDrive->local()->uploadCert($targetPath ,$realUpload);
        return  $img;
    ...
    private function actionUploadImage( $imagePath,$localPath='')
    {
        $cosComponent =   new AliOssComponent(ArrayHelper::merge($this->config, [])  );
        $cosFolder=$imagePath .'/'. date('Ymd', time());
        if(empty($localPath)){
            $file = UploadedFile::getInstanceByName('file');
            $fileData = $cosComponent->upload($cosFolder, $file);
        }else{
            $fileData = $cosComponent->uploadPath($cosFolder, $localPath);
        }
        if (empty($fileData)) {
